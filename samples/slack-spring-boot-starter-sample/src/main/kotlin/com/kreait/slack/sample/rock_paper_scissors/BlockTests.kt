...
class BlockTests @Autowired constructor(private val slackClient: SlackClient) : SlashCommandReceiver {
    ...
    override fun onReceiveSlashCommand(slackCommand: SlackCommand, headers: HttpHeaders, team: Team) {
        ...
        this.slackClient.respond().message(slackCommand.responseUrl)
                .with(assembleBlockRequest(slackCommand.channelId, slackCommand.userId))
                .onSuccess { println(it) }
                .onFailure { println(it) }
                .invoke()
    }

    private fun assembleBlockRequest(channelId: String, userId: String): RespondMessageRequest {
        val section = Block.Section(Text(Text.Type.PLAIN_TEXT, "this is a section"),
                fields = listOf(Text(Text.Type.PLAIN_TEXT, "it can contain "),
                        Text(Text.Type.PLAIN_TEXT, "many text-fields")), blockId = "section")
    ...
    private fun assembleElements(): List<Element> {
        return listOf(
                ...
                Element.UsersSelect(actionId = "user_select", placeholderText = Text(Text.Type.PLAIN_TEXT, "Select a User")),
                Element.StaticSelect(actionId = "static_select", placeholderText = Text(Text.Type.PLAIN_TEXT, "select something"),
                        options = listOf(Option(Text(Text.Type.PLAIN_TEXT, "first entry"), value = "first_entry"),
                ...
                Element.ConversationsSelect(actionId = "convo_select", placeholderText = Text(Text.Type.PLAIN_TEXT, "select a conversation")),
                Element.ChannelsSelect(actionId = "channel_select", placeholderText = Text(Text.Type.PLAIN_TEXT, "select a channel")),
                Element.DatePicker(actionId = "date_picker"),
                Element.Overflow(actionId = "overflow",
                        options = listOf(Option(Text(Text.Type.PLAIN_TEXT, "option1"), "option1"),
