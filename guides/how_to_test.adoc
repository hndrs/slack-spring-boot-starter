...
We use the MockSlackClient to set the correct response for our testcase, since the respond().message() doesn't have a return value, we just set Unit in our `testSuccess()` method.
